...
data Arrow
  = UnrestrictedArrow
  | LinearArrow
  | ExplicitMult (WithComments Type)

instance CommentExtraction Arrow where
  nodeComments UnrestrictedArrow = NodeComments [] [] []
  nodeComments LinearArrow = NodeComments [] [] []
  nodeComments (ExplicitMult mult) = nodeComments mult

instance Pretty Arrow where
  pretty' UnrestrictedArrow = string " -> "
  pretty' LinearArrow = string " %1 -> "
  pretty' (ExplicitMult mult) = string " %" >> pretty mult >> string " -> "
...
instance Pretty Type where
  ...
  pretty' Function {..} = (pretty from >> pretty arrow) |=> pretty to
...
mkArrow :: GHC.HsArrow GHC.GhcPs -> Arrow
mkArrow (GHC.HsUnrestrictedArrow _) = UnrestrictedArrow
mkArrow (GHC.HsLinearArrow _) = LinearArrow
#if MIN_VERSION_ghc_lib_parser(9, 8, 1)
mkArrow (GHC.HsExplicitMult _ mult _) = ExplicitMult (mkType <$> fromGenLocated mult)
#else
mkArrow (GHC.HsExplicitMult _ mult) = ExplicitMult (mkType <$> fromGenLocated mult)
#endif
...
instance Pretty VerticalFuncType where
  pretty' (VerticalFuncType Function {..}) = do
    ...
    newline
    let arrowStr = case arrow of
          UnrestrictedArrow -> "-> "
          LinearArrow -> "%1 -> "
          ExplicitMult _ -> error "Cannot format ExplicitMult in vertical function type"
    prefixed arrowStr $ pretty $ fmap mkVerticalFuncType to
    ...
    where
      ...
      ver = do
        ...
        let arrowStr = case arrow of
              UnrestrictedArrow -> "-> "
              LinearArrow -> "%1 -> "
              ExplicitMult mult -> error "Cannot format ExplicitMult in vertical DeclSigType"
        prefixed arrowStr $ pretty $ mkVerticalFuncType <$> to
