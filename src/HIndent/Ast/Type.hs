...
mkTypeFromConDeclField field =
  case (unpack, bangness) of
    (GHC.NoSrcUnpack, GHC.NoSrcStrict) -> base
    _ ->
      addComments
        (getComments base)
        (mkWithComments StrictType {bang = bang, baseType = base})
  where
    fieldType = GHC.cdf_type field
    unpack = GHC.cdf_unpack field
    bangness = GHC.cdf_bang field
    base = mkType <$> fromGenLocated fieldType
    bang = mkBang (GHC.HsSrcBang SourceText.NoSourceText unpack bangness)
#else
