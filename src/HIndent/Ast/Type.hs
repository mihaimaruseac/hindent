...
mkTypeFromConDeclField GHC.CDF {..}
  | GHC.NoSrcUnpack <- cdf_unpack
  , GHC.NoSrcStrict <- cdf_bang = baseType
  | otherwise =
    addComments (getComments baseType) (mkWithComments StrictType {..})
  where
    baseType = mkType <$> fromGenLocated cdf_type
    bang = mkBang (GHC.HsSrcBang SourceText.NoSourceText cdf_unpack cdf_bang)
#else
